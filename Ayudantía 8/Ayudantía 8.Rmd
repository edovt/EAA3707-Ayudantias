...
3.  Random Forests
4.  Búsqueda iterativa de hiperparámetros
...
En este caso hay dos métricas que son más de interés: **precision** y **recall**. Juntamos todo lo anterior en el código a continuación, utilizando ahora estas métricas.
...
En la sección anterior asumimos que los datos son linealmente separables, o que al menos podíamos utilizar las variables slack tal que nos quedaran de esa manera. Dado los resultados obtenidos en la parte aplicada anterior, es posible que no existían maneras óptimas de incluir estas variables para así obtener una solución.

¿Qué alternativas tenemos entonces?: lo que podemos hacer es encontrar un límite de decisión que no sea lineal.

La forma en que hacemos lo anterior es transformando los datos de entrada en un espacio de mayor dimensión en el cual sí podamos encontrar un separador lineal. El problema de lo anterior es que el costo de resolver el problema de optimización en este nuevo espacio es demasiado alto.

De todas maneras, es posible mostrar que el nuevo problema de optimización es equivalente al anterior, pero que ahora se cambian los productos punto $x_i^Tx_j$ por los productos punto en el nuevo espacio $\phi(x_i)^T\phi(x_j)$. ¿Qué nos dice esto?, esto nos dice que lo único que necesitamos realmente para resolver el problema es una función que dado dos valores $x_i$ y $x_j$ nos entregue el producto punto en el nuevo espacio, y esta función es la que se denomina **kernel**, denotada por

$$
K(x_i, x_j) = \phi(x_i)^T\phi(x_j) 
$$

En nuestro caso veremos dos kernels que están implementados en `parsnip`: el polinomial y el gaussiano (también conocido como RBF de Radial Basis Functions)

## Modelamiento

En nuestro caso utilizaremos el kernel gaussiano para mostrar las diferencias con el caso lineal. Definimos el modelo igual que antes, solo que ahora usamos `svm_rbf`. En este caso el único motor que acepta `parsnip` es `kernlab` y considera dos hiperparámetros: el costo $C$ y el parámetro del kernel $\sigma$.
...
En las ayudantías pasadas hemos realizado el tuning de los hiperparámetros de la misma manera: con grillas regulares. Es importante notar que esta no es la única forma, y que existen otras cada una con sus ventajas y desventajas.

En particular, existen dos modos: búsqueda en grillas y búsqueda iterativa. En este caso nos enfocaremos en la búsqueda en grilla, la cual tiene subdivisiones en regulares e irregulares.

Veamos por ejemplo en nuestro caso las diferencias:
...
Sin un motivo específico, utilizaremos la grilla de máxima entropía.
...
Algo importante de lo anterior es que notamos que el **accuracy** se mantiene constante, en un valor bastante alto, al cambiar **ambos** hiperparámetros. Repetimos el análisis anterior.
...
¿Ocurrirá lo mismo que la vez pasada?
...
**Repitan lo mismo con `rbf_poly`**

# Discusión: problemas multiclase
