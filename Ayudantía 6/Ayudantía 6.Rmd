...
```{r}
bankruptcy <- read_csv(here("Ayudantía 6", "bankruptcy.csv"),
...
```{r}
bankruptcy <- bankruptcy[, c(1, 2, 5, 9, 10, 14, 19, 24, 29, 33, 37, 48, 51, 56, 66, 72)]
```
...
Como lo hemos hecho en las ayudantías pasadas, realizamos los primeros pasos típicos con tidymodels en el código a continuación. Es importante que para esta ayudantía utilizaremos `svm_linear`, donde para la siguiente exploraremos las otras opciones.

Para la especificación del modelo, tunearemos el parámetro de costo $C$, como aparece más arriba en el problema de optimización.
